...
export function CurrentSubscriptionCard({ subscription }: { subscription: Subscription | null }) {
  ...
  const isFree = subscription.plan.priceInCents === 0
        ...
        <div className="divide-y">
          <div className="flex items-center justify-between py-3 first:pt-0">
            ...
            <span className="flex items-center gap-1.5 text-sm font-medium capitalize">
              <span className={`h-2 w-2 rounded-full ${subscription.status === 'active' ? 'bg-green-500' : 'bg-muted-foreground'}`} />
              {subscription.status}
            </span>
          </div>
          ...
          {subscription.plan.connectsPerPeriod && (
            <div className="flex items-center justify-between py-3">
              <span className="text-sm text-muted-foreground">Monthly Contacts</span>
