...
async function main() {
  ...
  const existingClients = await prisma.user.count({
  ...
  // ── Seed notification preferences for demo users ──
  const allDemoUsers = [...clientRecords, ...talentRecords]
  ...
  console.log(`  Seeded notification preferences for ${allDemoUsers.length} demo users`)
  ...
  // ── Demo subscription for VA Talent Co. ──
